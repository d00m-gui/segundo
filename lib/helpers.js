 ...
 * Setup
 *
 * This is the style and configuration of a Segundo Application.
 * returns object with contents of Segundo.json and application style
 *
 *   {
...
exports.setup = function(projectPath, env){
  ...
  try{
    var configPath  = path.join(projectPath, "segundo.json")
    var contents    = fs.readFileSync(configPath).toString()
    var publicPath  = path.join(projectPath, "public")
  }catch(e){
    try{
      var configPath  = path.join(projectPath, "_segundo.json")
      var contents    = fs.readFileSync(configPath).toString()
...
exports.willAllow = function(projectPath, outputPath){
  ...
  if(willCollide(projectPath, outputPath)){
    if(relativePath === ".."){
      if(projectPath.split("/")[projectPath.split("/").length - 1][0] == "_"){
        return true
      }else{
        return false
      }
